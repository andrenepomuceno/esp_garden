...
| `io` | 1 s | background | All ADC reads, `webUpdateDataCache()`, then the relay / float / **cloud** / **step-value** events. The cloud and ET0 models ride this tick rather than taking slots of their own |
...
- **Changing `ota.password` is pushed into `UserStore` too.** The login password lives in `/users.json`, so a config-only change would not take effect until a filesystem deploy wiped the user store.
