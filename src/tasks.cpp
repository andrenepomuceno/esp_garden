...
// Arms the post-watering check, called by publishRelayEvents() the second a
// zone's pump starts.
//
// It exists as a function because the task OBJECT does not leave this file.
// g_checkMoistureTask, and every other g_*Task, is static here, so the one
// translation unit that holds the registration order in tasksSetup() is also
// the only one that can arm, park or re-period a task — there is no second
// place where a reader has to go looking for what touches the scheduler. The
// library documents mutation after start() as unsafe; keeping the objects
// private is what makes "who mutates them" a question with a one-file answer.
void
armMoistureCheck()
{
